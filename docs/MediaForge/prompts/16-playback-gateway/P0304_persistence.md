# P0304 — Unified playback sessions, gateway and stream routing: persistence
...
- Persist durable user/device/library/item audio profiles and versioned loudness-analysis results where this prompt's schema work requires them; do not persist every slider movement as high-volume telemetry.
- Preserve the precedence defined by the canonical audio policy and keep ephemeral playback-session overrides ephemeral.

## Exact work for this prompt

1. Inspect the existing implementation specifically for **Unified playback sessions, gateway and stream routing** and the current focus **persistence**.
...
- **Tests run + results**
- **Behavior guaranteed after P0304**
