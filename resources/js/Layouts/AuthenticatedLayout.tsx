...
import { CatalogIcon, ConnectorsIcon, DashboardIcon, ImportIcon, LibraryIcon, ReviewIcon, ServerIcon, SettingsIcon, SyncIcon } from '@/Components/UI/Icon';
...
const NAV = [
    ...
    { href: '/catalog', label: 'External Catalog', icon: <CatalogIcon className="size-4" /> },
    { href: '/imports', label: 'Import Plans', icon: <ImportIcon className="size-4" /> },
...
function breadcrumb(url: string): [string, string] {
    ...
    if (url.startsWith('/catalog')) return ['External Catalog', 'Read-only snapshots'];
    if (url.startsWith('/imports')) return ['Import Plans', 'Dry run only'];
